...
uint32_t fat32Open( const char* filePath, file_id* fileId )
{
	...
	fd->currentFatSectorBuffer = (uint8_t*)malloc( _fat32Bps.bytes_per_sector );
	memset( fd->currentFatSectorBuffer, 0, _fat32Bps.bytes_per_sector );

	fd->currentClusterBuffer = (uint8_t*)malloc( sizeof(_clusterBufferSize) );
...
uint32_t loadFsRoot( void )
{
	...
	_clusterBuffer = (uint8_t *)malloc( _clusterBufferSize );
...
void readDirectory( uint8_t* buffer, DIR_ENTRY* dir )
{
	...
	dir->children = (DIR_ENTRY*)malloc( dir->childrenCount * sizeof( DIR_ENTRY ) );
